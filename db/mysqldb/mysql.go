...
type option struct {
	...
	ConnMaxLifeSecond  time.Duration //最大的连接时间
	PrepareStmt        bool          //预编译
	LogName            string
	SlowLogMillisecond int64 //慢日志查询阈值时间
	EnableSqlLog       bool  //sql语句日志
}
...
var (
	mysqlClients  = make(map[string]*DB) //在实际项目中，可用到多个mysql，这边采用map k/v
...
func dbConnect(user, pass, host, dbName string, option *option) (*gorm.DB, error) {
	...
	// 设置最大连接数 用于设置闲置的连接数.设置闲置的连接数则当开启的一个连接使用完成后可以放在池里等候下一次使用。提高执行效率
