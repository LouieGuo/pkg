...
func TestMongo(t *testing.T) {
	//初始化mongoDB
	//获取mongodb的连接客户端
	err := InitMongoClient(DefaultMongoClient, "admin", "admin123", []string{"192.168.161.130:27017"}, 100)
	if err != nil {
		MongoStdLogger.Println(err)
		panic(err)
	}
	mongoClient := GetMongoClient(DefaultMongoClient)
	//1.插入
	//user := User{
	//	ID:   1,
	//	Name: "test1",
	...
	if err != nil {
		MongoStdLogger.Print("CreateIndex : ", " error : ", err)
	}
}
